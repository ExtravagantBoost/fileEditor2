 /*global l,bootstrap,JSHINT,stylelint,langd,bs,commonmark,require,Terser*/
...
var Fe2 = new(class {
  constructor() {
    try {
      ...
      let files = {
        ...
        update: (name, content, errs = null) => {
          ...
          if (typeof content != "string")
            throw aerr(
              `content must be String, not ${typeof content}`
            );
      ...
      Object.defineProperty(files, "current", {
        ...
        set: (v) => {
          ...
          if (self.editor) {
            ...
            self.files.get(fh[1]).scroll.top =
              self.editor.scrollDOM.scrollTop;
            ...
            self.editor.state.values[0].done =
              self.files.get(v).history.done;
            self.editor.state.values[0].undone =
              self.files.get(v).history.undone;
            self.editor.scrollDOM.scrollTop =
              self.files.get(v).scroll.top;
            self.editor.scrollDOM.scrollLeft =
              self.files.get(v).scroll.left;
            self.editor.focus();
          ...
          files._().forEach((entry) => {
            ...
            let menu = presetelements.dropdown(fn, [{
                li: [{
                  ...
                  class: "dropdown-item",
                  style: {
                    "--bs-dropdown-link-hover-bg": "rgba(var(--bs-primary-rgb),0.7)",
                    transition: "background 0.25s",
                  },
                  on: {
                    click: () => {
                      /*global saveAs*/
                      let fl = new Blob([cache[1]], {
                        type: `${detectMime(cache[0])||"text/plain"};charset=utf-8`
                      });
                      saveAs(fl, cache[0]);
                    },
                  },
              ...
              },
              {
                li: [{
                  ...
                  class: "dropdown-item",
                  style: {
                    "--bs-dropdown-link-hover-bg": "rgba(var(--bs-danger-rgb),0.7)",
                    transition: "background 0.25s",
                  },
                  on: {
                    click: (event) => {
      ...
      const {
        indentationMarkers
      } = require("@replit/indent-markers");
      const {
        color
      } = require("@uiw/color-extension");
      const {
        detectMime
      } = require("@Fe2/mime");
      const {
      ...
      Object.defineProperties(terserify, {
        ...
        run_cli: {
          value: () => {
            console.warn(
              "this function is not available in browser env"
            );
          },
        },
      });
      ...
      function handle(
        ...
        if (typeof key != "string")
          throw new TypeError(
            "key must be string, not " + typeof key
          );
        return keymap.of({
          key,
          run,
      ...
      if (window.location.href.includes(".preview.app") || window.location.href.includes("file://")) {
                    ...
                    FR.onload = (ev) => {
                      try {
                        ...
                        let fileManifest = {
                          ...
                          fileObject: file || new Blob([fileName], {
                            type: `${detectMime(fileName)||"text/plain"};charset=utf-8`
                        ...
                        if (self.files.get(fileName)) {
                              ...
                              },
                              {
                                ...
                                on: {
                                  click: () => {
                                    ...
                                    self.files.update(fileManifest.fileName, fileManifest.contents, false);
                                  },
                                },
                              },
                              {
                                ...
                                on: {
                                  click: () => {
                                    self.files.create(fileManifest.fileName, fileManifest.contents, fileManifest, true);
                          ...
                          self.files.create(fileManifest.fileName, fileManifest.contents, fileManifest, false);
